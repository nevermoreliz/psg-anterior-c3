...
class Reporte_matriculacion extends FPDF
{
    public function __construct()
    {
        parent::__construct();
        $this->CI = &get_instance();
        // $this->CI->load->model('matriculacion_model');
    }

    public function matricula_pdf($matricula)
    {
        $this->AddPage('P', 'Letter');
        $this->fuentepdf = 'helvetica';
        if (substr($matricula->fecha_matriculacion, 0, 4) === '2021') {
            $matricula->antiguedad = $matricula->categoria . '/' . $matricula->anio_ingreso;
        } else {
            $matricula->antiguedad = $matricula->categoria . '/' . $matricula->gestion_programa;
        }
        $this->frontal_matricula($matricula, -3);
        $this->frontal_matricula($matricula, 98);
        $this->anverso_matricula($matricula, -3, 5);
        $this->anverso_matricula($matricula, 98, 5);
        $this->anverso_matricula($matricula, -3, 80);
        $this->anverso_matricula($matricula, 98, 80);
        return $this->Output('S');
    }

    private function frontal_matricula($persona, $x = 0)
    {
        $this->SetFont($this->fuentepdf, '', 6);
        $this->SetFont('', 'B', 6);
        $this->SetY(26);
        $this->SetX(10 + $x);
        $this->SetFont('', '', 8);
        $this->Cell(95, 5, utf8_decode($persona->paterno . ' ' . $persona->materno . ',' . $persona->nombre), 0, 1, 'C');
        $this->Cell(95, 5, '', 0, 1, '');
        $this->SetX(10 + $x);
        $this->Cell(28, 5, '', 0, 0, '');
        $this->Cell(27, 5, ($persona->ci . ' ' . $persona->expedido), 0, 0, 'C');
        $this->Cell(40, 5, ($persona->registro_universitario), 0, 1, 'C');
        $this->Cell(95, 5, '', 0, 1, '');
        $this->SetX(10 + $x);
        $this->Cell(28, 5, '', 0, 0, '');
        $this->Cell(67, 5, (/*$persona->nombre_unidad*/'CEFORPI - UPEA'), 0, 1, 'C');
        $this->Cell(95, 5, '', 0, 1, '');
        $this->SetX(10 + $x);
        $this->Cell(28, 5, '', 0, 0, '');
        $this->Cell(34, 5, utf8_decode($persona->descripcion_grado_academico), 0, 0, 'C');
        $this->Cell(33, 5, ($persona->antiguedad), 0, 1, 'C');
        $this->Cell(95, 6, '', 0, 1, '');
        $this->SetX(10 + $x);
        $this->Cell(28, 5, '', 0, 0, '');
        $this->Cell(5, 5, strftime('%d', strtotime($persona->fecha_matriculacion)), 0, 0, 'C');
        $this->Cell(10, 5, '', 0, 0, '');
        $this->Cell(20, 5, strftime('%B', strtotime($persona->fecha_matriculacion)), 0, 0, 'C');
        $this->Cell(10, 5, '', 0, 0, '');
        $this->Cell(22, 5, strftime('%y', strtotime($persona->fecha_matriculacion)), 0, 1, 'L');
    }

    private function anverso_matricula($matricula, $x = 0, $y = 0)
    {
        $this->SetFont($this->fuentepdf, '', 6);
        $this->SetY(92 + $y);
        $this->SetX(10 + $x);
        $this->SetFont('', '', 8);
        $this->Cell(95, 5, utf8_decode($matricula->paterno . ' ' . $matricula->materno . ',' . $matricula->nombre), 0, 1, 'C');
        $this->Cell(95, 5, '', 0, 1, '');
        $this->SetX(10 + $x);
        $this->Cell(50, 5, ($matricula->ci . ' ' . $matricula->expedido), 0, 0, 'C');
        $this->Cell(45, 5, ($matricula->registro_universitario), 0, 1, 'C');
        $this->Cell(95, 5, '', 0, 1, '');
        $this->SetX(10 + $x);
        $this->Cell(95, 5, (/*$matricula->nombre_unidad*/'CEFORPI - UPEA'), 0, 1, 'C');
        $this->Cell(95, 5, '', 0, 1, '');
        $this->SetX(10 + $x);
        $this->Cell(50, 5, utf8_decode($matricula->descripcion_grado_academico), 0, 0, 'C');
        $this->Cell(45, 5, ($matricula->antiguedad), 0, 1, 'C');
        $this->Cell(95, 10, '', 0, 1, '');
        $this->SetX(10 + $x);
        $this->Cell(13, 3, '', 0, 0, '');
        $this->Cell(10, 3, strftime('%d', strtotime($matricula->fecha_matriculacion)), 0, 0, 'C');
        $this->Cell(26, 3, strftime('%B', strtotime($matricula->fecha_matriculacion)), 0, 0, 'C');
        $this->Cell(4, 3, '', 0, 0, '');
        $this->Cell(10, 3, strftime('%y', strtotime($matricula->fecha_matriculacion)), 0, 0, 'L');
        $this->Cell(10, 3, '', 0, 0, '');
        $this->Cell(16, 3, $matricula->costo_matricula, 0, 1, 'C');
    }
    ...
    // }
    public function formulario_inscripcion($doc)
    {
        $this->AddPage('P', 'Letter');
        ...
        $this->Image('img/membrete.jpg', 0, 0, 216, 279);
        // $this->Image('https://chart.googleapis.com/chart?chs=100x100&cht=qr&chl=' . str_replace(' ', '_', ($doc['id_inscripcion'] . '/' . $doc['id_planificacion_programa'] . '/' . $doc['ci'] . $doc['expedido'] . '/' . $doc['paterno'] . '/' . $doc['nombre'] . '.png')), 180, 230, 20);
        // $this->Image('https://barcode.tec-it.com/barcode.ashx?data=' . ($doc['id_inscripcion'] . '/' . $doc['id_planificacion_programa'] . '/' . $doc['ci'] . $doc['expedido']) . '&code=Code128&dpi=96&dataseparator=', 120, 235, 50, 0, 'GIF');
        $this->SetFont('RubikB', '', 18);
        $this->Cell(0, 8, utf8_decode('DIRECCIÓN DE POSGRADO'), '', 1, 'R');
        $this->Cell(0, 8, 'CEFORPI - UPEA', '', 1, 'R');
        $this->Ln();
        $this->SetFont('Rubik', '', 24);
        $this->Cell(0, 8, 'FORMULARIO DE', ', 1', 1, 'R');
        $this->Cell(0, 8, utf8_decode('INSCRIPCIÓN'), ', 1', 1, 'R');
        $this->SetFont('Rubik', '', 11);
        $this->Cell(0, 8, 'Fecha de Registro: ' . fecha_literal($doc['fecha_inscripcion'], 2), ', 1', 1, 'R');
        $this->SetFont('RubikB', '', 14);
        $this->Cell(0, 8, 'DATOS DEL PROGRAMA', '', 1, 'L');
        $this->SetFont('RubikB', '', 10);
        $this->Cell(36, 8, utf8_decode('DENOMINACIÓN: '), '', 0, 'L', TRUE);
        $this->SetFont('Rubik', '', 10);
        $this->MultiCell(0, 8, utf8_decode($doc['descripcion_grado_academico'] . ' EN ' . $doc['nombre_programa']), '', 'L', TRUE);
        $this->Ln(1);
        $this->SetFont('RubikB', '', 10);
        $this->Cell(30, 8, 'MODALIDAD: ', '', 0, 'L', TRUE);
        $this->SetFont('Rubik', '', 10);
        $this->Cell(30, 8, $doc['nombre_tipo_programa'], '', 0, 'L', TRUE);
        $this->SetFont('RubikB', '', 10);
        ...
        $this->SetFont('Rubik', '', 10);
        $this->Cell(48, 8, $doc['denominacion_sede'], '', 1, 'L', TRUE);
        $this->Ln(4);
        $this->SetFont('RubikB', '', 14);
        ...
        $this->SetFont('Rubik', '', 10);
        $this->MultiCell(0, 8, $doc['nombre'] . ' ' . $doc['paterno'] . ' ' . $doc['materno'], '', 'L', TRUE);
        $this->Ln(1);
        $this->SetFont('RubikB', '', 10);
        $this->Cell(45, 8, 'FECHA DE NACIMIENTO: ', '', 0, 'L', TRUE);
        $this->SetFont('Rubik', '', 10);
        $this->Cell(48, 8, fecha_literal($doc['fecha_nacimiento'], 1), '', 0, 'L', TRUE);
        $this->SetFont('RubikB', '', 10);
        ...
        $this->SetFont('Rubik', '', 10);
        $this->Cell(65, 8, $doc['email'], '', 1, 'L', TRUE);
        $this->Ln(1);
        $this->SetFont('RubikB', '', 10);
        $this->Cell(45, 8, 'OFICIO DE TRABAJO: ', '', 0, 'L', TRUE);
        $this->SetFont('Rubik', '', 10);
        $this->MultiCell(0, 8, $doc['oficio_trabajo'], '', 'L', TRUE);
        $this->Ln(1);
        // $this->SetFont('RubikB', '', 10);
        ...
        $this->SetFont('Rubik', '', 10);
        $this->MultiCell(0, 8, $doc['domicilio'], '', 'L', TRUE);
        $this->Ln(4);
        $this->SetFont('RubikB', '', 14);
        $this->Cell(0, 8, utf8_decode('DEPÓSITO BANCARIO POR MATRICULACIÓN'), '', 1, 'L');
        $this->SetFont('RubikB', '', 10);
        $this->Cell(40, 8, utf8_decode('NRO. DE DEPÓSITO: '), '', 0, 'L', TRUE);
        $this->SetFont('Rubik', '', 10);
        $this->Cell(33, 8, $doc['numero_deposito_matricula'], '', 0, 'L', TRUE);
        $this->SetFont('RubikB', '', 10);
        $this->Cell(40, 8, utf8_decode('FECHA DE DEPÓSITO: '), '', 0, 'L', TRUE);
        $this->SetFont('Rubik', '', 10);
        $this->Cell(53, 8, fecha_literal($doc['fecha_deposito_matricula'], 1), '', 1, 'L', TRUE);
        $this->Ln(4);
        $this->SetFont('RubikB', '', 14);
        $this->Cell(0, 8, utf8_decode('DEPÓSITO BANCARIO POR COLEGIATURA'), '', 1, 'L');
        $this->SetFont('RubikB', '', 10);
        $this->Cell(40, 8, utf8_decode('NRO. DE DEPÓSITO: '), '', 0, 'L', TRUE);
        $this->SetFont('Rubik', '', 10);
        $this->Cell(33, 8, $doc['numero_deposito_cuota_inicial'], '', 0, 'L', TRUE);
        $this->SetFont('RubikB', '', 10);
        $this->Cell(40, 8, utf8_decode('FECHA DE DEPÓSITO: '), '', 0, 'L', TRUE);
        $this->SetFont('Rubik', '', 10);
        $this->Cell(53, 8, fecha_literal($doc['fecha_deposito_inicial'], 1), '', 1, 'L', TRUE);
        $this->Ln(4);
        $this->SetFont('Rubik', '', 7);
        ...
        $this->SetFont('Rubik', '', 9);
        $this->SetXY(30, 235);
        $this->Cell(0, 4, $doc['nombre'] . ' ' . $doc['paterno'] . ' ' . $doc['materno'], '', 1, 'L');
        $this->Cell(0, 4, $doc['ci'] . ' ' . $doc['expedido'], '', 1, 'L');
        return $this->Output('S');
    }

    public function formulario_01($doc)
    {
        ...
        $this->SetFont('helvetica', '', 9);
        $this->Text(81, $alto, utf8_decode($doc['denominacion_sede']));
        /**Fin 3*/
        /**---Fin Seccion 4---*/
    ...
    public function imprimir_carta_compromiso($doc)
    {
        ...
        $this->AddPage('P', 'Letter');
        $this->Image('img/solicitud.jpg', 0, 0, 216, 279);
        $this->SetFont('Rubik', '', 12);
        $this->Cell(0, 8, 'El Alto, ' . fecha_literal($doc['fecha_inscripcion'], 1), 0, 1, 'R');
        $this->Cell(0, 8, utf8_decode('Señor:'), 0, 1, 'L');
        $this->Cell(0, 8, 'M. Sc. Richard Jorge Torrez Juaniquina', 0, 1, 'L');
        $this->SetFont('RubikB', '', 11);
        $this->Cell(0, 8, 'DIRECTOR DE POSGRADO - UPEA', 0, 1, 'L');
        $this->SetFont('Rubik', '', 12);
        $this->Cell(0, 8, 'Presente.-', 0, 1, 'L');
        $this->Ln(16);
        $this->SetFont('RubikB', 'U', 12);
    ...
    public function imprimir_solicitud_inscripcion($doc)
    {
        ...
        $this->Ln(5);
        $this->MultiCell(0, 8, utf8_decode('El motivo de la presente es para solicitar a su autoridad la INSCRIPCIÓN AL PROGRAMA: ' . $doc['descripcion_grado_academico'] . ' EN ' . $doc['nombre_programa'] . ' - VERSIÓN ' . $doc['numero_version'] . '.'), '', 'FJ');
        $this->Ln(5);
        $this->Cell(0, 8, utf8_decode('Sin otro particular me despido con las consideraciones más distinguidas.'), 0, 1, 'L');
        $this->Ln(5);
        $this->Cell(156, 8, 'Atentamente,', 0, 1, 'L');
