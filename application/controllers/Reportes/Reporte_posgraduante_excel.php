...
class Reporte_posgraduante_excel extends Spreadsheet
{
    ...
    public function lista_posgraduante($data=null)
    {
		...
		$contador = 13; 
		$x=1;		
        //$this->getActiveSheet()->getStyle("A" . $contador . ":T" . $contador)->getAlignment()->setWrapText(true);
        ...
        $filename = 'reporte_inscritos_' . str_replace(array(' ', '|'), '_', 'ddddddd');
		...
		$sheet->setCellValue('A8', 'Hola Mundo !');
		header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachment;filename="'.utf8_decode('ffffff').'.xlsx"');
            header('Cache-Control: max-age=0');
