...
$(document).ready(function () {
	...
	$('.validation-wizard').steps({
		...
		onStepChanging: function (event, currentIndex, newIndex) {
			return (
				...
				(!(3 === newIndex && Number($('#age-2').val()) < 18) &&
					(currentIndex < newIndex &&
						(form.find('.body:eq(' + newIndex + ') label.error').remove(), form.find('.body:eq(' + newIndex + ') .error').removeClass('error')),
		...
		onFinished: function (event, currentIndex) {
			...
			$.ajax({
					type: 'POST',
					url: '/admin_marketing/publicacion_insertar',
					dataType: 'json',
					data: formData,
					// data: new FormData($("#form_campos_publicacion")[0]), 
					processData: false,
					contentType: false,
					cache: false,
					async: false,
				...
				.done(function (data) {
					...
					if (typeof data.exito !== 'undefined') {
						...
						swal('Programa publicado', 'haga click en "ok" para continuar');
	...
	$('#archivo').change(function () {
		...
		var element;
		var supportedImages = ['image/jpeg', 'image/jpg', 'image/png'];
		...
		if (seEncontraronElementoNoValidos) {
			alertaSimple('ERROR', 'Se Encontraron archivos no validos', 'top-right', 'error', 3000);
		} else {
			alertaSimple('EXITOSO', 'Todos Los Archivos Se Subieron Correctamente.', 'top-right', 'info', 4000);
		}
	});
